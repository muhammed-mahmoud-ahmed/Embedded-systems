...
{
	// DIO_vWritePinValue(port_name, 4, ((1 & port_value) >> 4)); // read bit 4
	// DIO_vWritePinValue(port_name, 5, ((1 & port_value) >> 5)); // read bit 5
	// DIO_vWritePinValue(port_name, 6, ((1 & port_value) >> 6)); // read bit 6
	// DIO_vWritePinValue(port_name, 7, ((1 & port_value) >> 7)); // read bit 7
	for (unsigned char i = 4; i < 8; i++)
	{
		DIO_vWritePinValue(port_name, i, ((port_value >> i) & 1));
	}
